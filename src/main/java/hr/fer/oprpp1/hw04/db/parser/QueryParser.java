...
public class QueryParser {
	...
	private void parse() {	
		...
		while(true) {
			...
			if(isTokenOfType(TokenType.STATISTICS)) {
				staticticsFlasg = true;
				this.nextToken();
				if(isTokenOfType(TokenType.CONNECTORS) == true){
					throw new RuntimeException("can not use and with statictics as last token");
				}
				
				if(isTokenOfType(TokenType.EOF) == false){
					throw new RuntimeException("Statictics should be last token");
				}
			}
	...
	public boolean getStatictics() {
		return staticticsFlasg;
	}
	...
	private void parseExpresion() {
		...
		if(isTokenOfType(TokenType.NOT)) {
			negate = true;
			this.nextToken();
		}
		...
		ConditionalExpression localExpression = new ConditionalExpression(ifv, value, ico, negate);
